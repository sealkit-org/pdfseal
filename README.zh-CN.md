...
- **动态页码编排**：添加自定义页码（`{n}`、`{total}`、`Page {n} of {total}`、`第 {n} 页`），支持整行或局部白底色带遮罩覆盖原有错乱旧页码，支持羊皮纸与画布吸色，支持跳过首页封面。
...
- ✅ **签名工具体验升级**——简签一键批量应用到多页、本地常用签名/印章库、手机拍照签名自动去白底去阴影
- ✅ **页面整理深度升级**——Shift / 框选多选批量旋转与删除、插入空白 A4 页（多语言水印）或追加外部文件、30 步撤销重做

**规划中**

- 🔜 **独立页码编排**——灵活宏格式（`Page {n} of {total}`）、排除封面、页眉页脚 6 个对齐锚点
- 🔜 **目标体积智能压缩**——输入期望体积（如 ≤ 2MB），二分法自动逼近上限并保持最高清晰度
- 🔜 更多界面语言（欢迎社区贡献！）
