<template>
  <section class="w-full flex-1 flex flex-col">
    ...
    <div class="bg-white rounded-2xl sm:rounded-3xl p-3.5 sm:p-7 shadow-xl border border-slate-100 flex flex-col flex-1">
          ...
          <div class="min-w-0">
            ...
            <div v-if="docBytes && !isProcessing && !lastExportedFile" class="flex items-center space-x-2 mt-0.5 min-w-0">
              ...
              <span 
                ...
                class="text-[10px] bg-violet-100 text-violet-800 px-1.5 py-0.5 rounded-md font-bold shrink-0 flex items-center"
              >
      ...
      >
        <div class="w-14 h-14 sm:w-16 sm:h-16 bg-violet-50 text-violet-600 rounded-2xl sm:rounded-3xl flex items-center justify-center mb-3 shadow-inner">
          <ListOrdered class="w-7 h-7 sm:w-8 sm:h-8" />
        </div>
        
        <h3 class="text-base sm:text-lg font-bold text-slate-800">{{ t('page_number_drop_title') }}</h3>
        <p class="text-xs text-slate-400 mt-1 max-w-sm hidden sm:block">{{ t('page_number_drop_subtitle') }}</p>
      ...
      <div v-else :class="['flex-1 flex flex-col justify-between min-h-0 overflow-hidden', isProcessing || lastExportedFile ? 'pt-4' : 'pt-2.5 sm:pt-3']">
        ...
        <div class="shrink-0 space-y-2.5 pt-2.5 sm:pt-3">
